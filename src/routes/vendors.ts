...
export async function vendorRoutes(req: Req, res: Res) {
  ...
  // /api/vendor/services (POST create)
  if (url.startsWith("/api/vendor/services") && method === "POST") {
    const user = getAuthUser(req);
    if (!user || (!user.role || user.role !== "vendor")) {
      send(res, 403, { message: "Only vendors can create services" });
      return;
    }
    const body = req.body || {};
    const id = uuidv4();
    await pool.query(
      "INSERT INTO collecto_vault_services (id,vendor_id,title,description,price_points,price_currency,active,image_url,created_at) VALUES (?,?,?,?,?,?,?,?,NOW())",
      [id, user.id, body.title, body.description, body.price_points || null, body.price_currency || null, typeof body.active === "boolean" ? (body.active ? 1 : 0) : 1, body.image_url || null]
    );
    send(res, 200, { id, message: "Service created" });
    return;
  }

  // GET /api/vendor/services or /api/vendor/services?vendorId=...
  if (url.startsWith("/api/vendor/services") && method === "GET") {
    const q = req.query || {};
    const vendorId = q.vendorId || (getAuthUser(req)?.id) || null;
    const [rows] = await pool.query("SELECT * FROM collecto_vault_services WHERE vendor_id = ?", [vendorId]);
    send(res, 200, rows);
    return;
  }
  ...
  if (matchService && method === "GET") {
    ...
    const [rows] = await pool.query("SELECT * FROM collecto_vault_services WHERE id = ? LIMIT 1", [id]);
  ...
  if (matchService && method === "PUT") {
    ...
    const id = matchService[1];
    const body = req.body || {};
    await pool.query("UPDATE collecto_vault_services SET title=?,description=?,price_points=?,price_currency=?,active=?,image_url=? WHERE id=?",
      [body.title, body.description, body.price_points || null, body.price_currency || null, body.active ? 1 : 0, body.image_url || null, id]);
  ...
  if (matchTier) {
    ...
    if (method === "GET") {
      const [rows] = await pool.query("SELECT * FROM collecto_vault_tiers WHERE vendor_id = ? ORDER BY min_points ASC", [vendorId]);
      send(res, 200, rows);
      return;
    }
    if (method === "POST") {
      const body = req.body || {};
      const id = uuidv4();
      await pool.query("INSERT INTO collecto_vault_tiers (id,vendor_id,name,min_points,color,badge_emoji,perks,auto_promote,expires_after_days,active,created_at) VALUES (?,?,?,?,?,?,?,?,?,1,NOW())",
        [id, vendorId, body.name, body.min_points || 0, body.color || null, body.badge_emoji || null, JSON.stringify(body.perks || []), body.auto_promote ? 1 : 0, body.expires_after_days || null]);
      send(res, 200, { id, message: "Tier created" });
      return;
    }
    if (method === "PUT" && tierId) {
      const body = req.body || {};
      await pool.query("UPDATE collecto_vault_tiers SET name=?,min_points=?,color=?,badge_emoji=?,perks=?,auto_promote=?,expires_after_days=?,active=? WHERE id=?",
        [body.name, body.min_points || 0, body.color || null, body.badge_emoji || null, JSON.stringify(body.perks || []), body.auto_promote ? 1 : 0, body.expires_after_days || null, body.active ? 1 : 0, tierId]);
      send(res, 200, { message: "Updated" });
      return;
    }
    if (method === "DELETE" && tierId) {
      await pool.query("DELETE FROM collecto_vault_tiers WHERE id = ?", [tierId]);
      send(res, 200, { message: "Deleted" });
      return;
    }
  }

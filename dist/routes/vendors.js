...
async function vendorRoutes(req, res) {
    const url = req.url || "";
    ...
    if (url.startsWith("/api/vendor/services") && method === "POST") {
        ...
        const body = req.body || {};
        const [result] = await connection_1.pool.query(`INSERT INTO collecto_vault_services 
    ...
    if (url.startsWith("/api/vendor/services") && method === "GET") {
        const q = req.query || {};
        const vendorId = q.vendorId || (0, auth_1.getAuthUser)(req)?.id || null;
        const [rows] = await connection_1.pool.query("SELECT * FROM collecto_vault_services WHERE vendor_id = ?", [vendorId]);
        send(res, 200, rows);
        return;
    }
    ...
    if (matchService && method === "GET") {
        ...
        const [rows] = await connection_1.pool.query("SELECT * FROM collecto_vault_services WHERE id = ? LIMIT 1", [id]);
    ...
    if (matchService && method === "PUT") {
        const user = (0, auth_1.getAuthUser)(req);
        if (!user)
            return send(res, 403, { message: "Unauthorized" });
        const id = matchService[1];
        const body = req.body || {};
        await connection_1.pool.query("UPDATE collecto_vault_services SET title=?,description=?,price_points=?,price_currency=?,active=?,image_url=? WHERE id=?", [
            body.title,
            body.description,
    ...
    if (matchService && method === "DELETE") {
        const id = matchService[1];
        await connection_1.pool.query("DELETE FROM collecto_vault_services WHERE id = ?", [id]);
    ...
    if (matchPR) {
        ...
        // GET RULES
        if (method === "GET") {
            const [rows] = await connection_1.pool.query("SELECT * FROM collecto_vault_pointrules WHERE vendor_id = ?", [vendorId]);
        ...
        const [rows] = await connection_1.pool.query("SELECT * FROM collecto_vault_vendors WHERE user_id = ?", [user.id]);
        ...
        if (method === "POST") {
            const body = req.body || {};
            await connection_1.pool.query(`INSERT INTO collecto_vault_pointrules 
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        if (method === "PUT" && ruleId) {
            const body = req.body || {};
            await connection_1.pool.query(`UPDATE collecto_vault_pointrules SET name=?, description=?, trigger_type=?, points=?, multiplier=?, max_per_transaction=?, max_per_day=?, active=?, start_at=?, end_at=? WHERE id=? AND vendor_id=?`, [
                body.name,
                body.description || null,
                body.trigger || "purchase",
                body.points || 0,
        ...
        if (method === "DELETE" && ruleId) {
            await connection_1.pool.query("DELETE FROM collecto_vault_pointrules WHERE id=? AND vendor_id=?", [ruleId, vendorId]);
    ...
    const matchTier = url.match(/^\/api\/vendor\/([^\/]+)\/tier-rules(?:\/([^\/]+))?/);
    if (matchTier) {
        const vendorId = matchTier[1];
        const tierId = matchTier[2];
        if (method === "GET") {
            const [rows] = await connection_1.pool.query("SELECT * FROM collecto_vault_tiers WHERE vendor_id = ? ORDER BY min_points ASC", [vendorId]);
            send(res, 200, rows);
            return;
        }
        if (method === "POST") {
            const body = req.body || {};
            await connection_1.pool.query("INSERT INTO collecto_vault_tiers (vendor_id,name,min_points,color,badge_emoji,perks,auto_promote,expires_after_days,active,created_at) VALUES (?,?,?,?,?,?,?,?,1,NOW())", [
                vendorId,
                body.name,
                body.min_points || 0,
                body.color || null,
        ...
        if (method === "PUT" && tierId) {
            const body = req.body || {};
            await connection_1.pool.query("UPDATE collecto_vault_tiers SET name=?,min_points=?,color=?,badge_emoji=?,perks=?,auto_promote=?,expires_after_days=?,active=? WHERE id=?", [
                body.name,
                body.min_points || 0,
                body.color || null,
                body.badge_emoji || null,
        ...
        if (method === "DELETE" && tierId) {
            await connection_1.pool.query("DELETE FROM collecto_vault_tiers WHERE id = ?", [
                tierId,
            ]);
            send(res, 200, { message: "Deleted" });
            return;
